...
pub fn filon_tab_sin(ftab: Vec<f64>, a: f64, b: f64, sin_coeff: f64) -> Result<f64, Error> {
    ...
    let s2n: f64 = (0..ftab.len()).step_by(2).map(|x| ftab[x] * sintab[x]).sum::<f64>()
               - 0.5*(ftab[ftab.len()]*sintab[ftab.len()] + ftab[0]*sintab[0]);
    let s2nm1: f64 = (1..ftab.len()-1).step_by(2).map(|x| ftab[x] * sintab[x]).sum();

    let output: f64 = h * (
        alpha * (ftab[0] * (sin_coeff*a).cos() - ftab[ftab.len()] * (sin_coeff*b).cos())
        + beta * s2n
        + gamma * s2nm1
    );
...
pub fn filon_tab_cos(ftab: Vec<f64>, a: f64, b: f64, cos_coeff: f64) -> Result<f64, Error> {
    ...
    let c2n: f64 = (2..ftab.len()-2).step_by(2).map(|x| ftab[x] * costab[x]).sum::<f64>()
          + 0.5*(ftab[ftab.len()]*costab[ftab.len()] + ftab[0]*costab[0]);
    let c2nm1: f64 = (1..ftab.len()-1).step_by(2).map(|x| ftab[x] * costab[x]).sum::<f64>();

    let output: f64 = h * (
        alpha * (ftab[0] * (cos_coeff*a).sin() - ftab[ftab.len()] * (cos_coeff*b).sin())
        + beta * c2n
        + gamma * c2nm1
    );
...
fn linspace(a: &f64, b: &f64, num_steps: usize) -> Vec<f64>{
    ...
    let step_size: f64 = (b - a) / num_steps as f64;
